...
const CD = (props) => {
    ...
    const audio_on_click = () => {
        props.setAudioPlaying(!props.audioPlaying);

        if (props.audioName != props.name) {
            props.setAudioName(props.name);
        }
    }
    ...
    return (
            ...
            <div style={{backgroundColor: props.cd_color, borderColor: props.bg_color, width: 0.337 * cd_size, height: 0.337 * cd_size, borderWidth: border_thickness}} onMouseEnter={(e) => {props.setPhotocardColor(props.albumData["mark_color"]); props.setFocusedSong(props.name); props.setCDCenterX(e.target.getBoundingClientRect().left + e.target.getBoundingClientRect().width/2); props.setCDCenterY(e.target.getBoundingClientRect().top + e.target.getBoundingClientRect().height/2);}} onMouseLeave={(e) => {props.setFocusedSong("");}} onMouseDown={() => {cd_mouse_down(); audio_on_click();}} className={`z-[1] rounded-[50%] hover:cursor-pointer flex items-center justify-center`}>
